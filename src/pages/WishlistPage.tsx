...
const WishlistPage = () => {
  ...
  return (
    <main className="bg-secondary/30 flex min-h-screen w-full justify-center">
      <SubNavbar />
      <section className="relative flex w-11/12 flex-col items-center justify-center py-20">
        ...
        <hr className="border-coffe/50 mb-5 w-full rounded-md border-b-2 border-solid sm:mb-0 sm:w-11/12" />
        <BtnDelete />
        ...
        <div
          className={`${wishlistProducts.length < 1 && "hidden"} mt-14 grid w-full grid-cols-1 place-content-center gap-3 py-4 sm:w-11/12 sm:grid-cols-2 sm:pl-6`}
        >
          {wishlistProducts.map((product: ProductCoffe) => (
            <ContentProductMenu key={product.id} item={product} />
          ))}
        </div>
        <hr className="border-coffe/50 mt-8 w-full rounded-md border-b-2 border-solid sm:mt-16 sm:w-11/12" />
        <NotifDeleteWishlist />
      </section>
    </main>
  );
